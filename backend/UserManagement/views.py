...
def UpdateInformationPage(request):
    ...
    user_profile = UserPro.objects.get(user=request.user)
...
def ViewProfile(request):
    if (request.user.is_authenticated == True):
        photo = None
        if (UserPro.objects.filter(user=request.user).exists()):
            photo = UserPro.objects.get(user=request.user)
        friends = UserPro.objects.all()
